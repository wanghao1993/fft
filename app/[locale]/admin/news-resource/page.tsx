...
import {
  ...
  getNewsResource,
  updateNewsResource,
} from "@/service/module/news_resource";
...
export default function AdminNewsPage() {
  const [news, setNews] = useState<NewsResource[]>([]);
  ...
  const renderCell = useCallback(
    (item: NewsResource, columnKey: string | number) => {
      if (columnKey === "category") {
        return item.category === "quick_news" ? "快讯" : "新闻";
  ...
  const getList = async () => {
    setIsLoading(true);
    try {
      const response = await getNewsResource();
  ...
  const handleSubmit = (data: CreateNewsResource) => {
    setIsLoading(true);
    if (id) {
      updateNewsResource(id, data).then(() => {
        getList();
        setIsLoading(false);
        setIsOpen(false);
        setId(null);
  ...
  return (
    <AuthWrapper>
      <h2 className="font-bold text-2xl mb-4 flex justify-between">
        <span className="mr-4">新闻资源管理</span>
        <Button
